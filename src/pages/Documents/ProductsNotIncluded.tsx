...
const ProductsNotIncluded = () => {
  const [pageNumber, setPageNumber] = useState(0);
  const [catalogCode, setCatalogCode] = useState("");
  const [count, setCount] = useState(0);
  const productsNotIncluded = useSelector(
    // @ts-ignore
    (state) => state.products.productsNotIncluded
  );
  const dispatch = useDispatch();
  console.log(productsNotIncluded);
  // @ts-ignore
  ...
  const fetchData = async (title, pageNumber) => {
    const res = await lgotaService.fetchProductNotIncluded(title, pageNumber);
    console.log(res);

    // @ts-ignore
    setCount(Math.ceil(res.count / 10));

    dispatch(productsAction.addNotIncluded(res.data));
  };

  console.log(productsNotIncluded[0]);
  ...
  const submitHandler = (e) => {
    ...
    lgotaService
      .fetchProductNotIncluded(title, pageNumber, catalogCode)
      .then((res) => dispatch(productsAction.addNotIncluded(res.data)));
  };
      ...
      <StyledTableContainer>
        <Table stickyHeader>
          ...
          <TableBody sx={{ position: "relative" }}>
            {/* @ts-ignore */}
            {productsNotIncluded?.map((row, index) => (
            ...
            <NoDataComponent isVisible={!productsNotIncluded?.length} />
          </TableBody>
        </Table>
      </StyledTableContainer>
...
export default ProductsNotIncluded;
