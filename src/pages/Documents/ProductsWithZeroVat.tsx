...
const ProductsWithZeroVat = () => {
  ...
  const productsWithZeroVat = useSelector(
    // @ts-ignore
    (state) => state.products.productsWithZeroVat
  );
  const dispatch = useDispatch();
  console.log(productsWithZeroVat);
  // @ts-ignore
  ...
  const fetchData = async (title, pageNumber) => {
    const res = await lgotaService.fetchZeroVatProduct(title, pageNumber);
    // @ts-ignore
    setCount(Math.ceil(res.count / 10));

    dispatch(productsAction.addWithZeroVat(res.data));
  };

  console.log(productsWithZeroVat[0]);
  ...
  const submitHandler = (e) => {
    ...
    lgotaService
      .fetchZeroVatProduct(title, pageNumber, tasnifName, lgotaName, tasnifCode)
      .then((res) => dispatch(productsAction.addWithZeroVat(res.data)));
  };
      ...
      <StyledTableContainer>
        <Table stickyHeader>
          ...
          <TableBody sx={{ position: "relative" }}>
            {/* @ts-ignore */}
            {productsWithZeroVat?.map((row, index) => (
            ...
            <NoDataComponent isVisible={!productsWithZeroVat?.length} />
          </TableBody>
        </Table>
      </StyledTableContainer>
...
export default ProductsWithZeroVat;
