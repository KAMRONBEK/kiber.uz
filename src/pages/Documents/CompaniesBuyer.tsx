...
const CompaniesBuyer = () => {
  const [pageNumber, setPageNumber] = useState(0);
  const [lgotaName, setLgotaName] = useState("");
  const [tin, setTin] = useState("");
  const [count, setCount] = useState(0);
  const companiesBuyerPriviliged = useSelector(
    // @ts-ignore
    (state) => state.products.companiesBuyerPriviliged
  );
  const dispatch = useDispatch();
  console.log(companiesBuyerPriviliged);
  // @ts-ignore
  ...
  const fetchData = async (title, pageNumber) => {
    const res = await lgotaService.fetchCompaniesBuyer(title, pageNumber);
    console.log(res);

    // @ts-ignore
    setCount(Math.ceil(res.count / 10));

    dispatch(productsAction.addCompaniesBuyer(res.data));
  };

  console.log(companiesBuyerPriviliged[0]);
  ...
  const submitHandler = (e) => {
    ...
    lgotaService
      .fetchCompaniesBuyer(title, pageNumber, tin, lgotaName)
      .then((res) => dispatch(productsAction.addCompaniesBuyer(res.data)));
  };
      ...
      <StyledTableContainer>
        <Table stickyHeader>
          ...
          <TableBody sx={{ position: "relative" }}>
            {/* @ts-ignore */}
            {companiesBuyerPriviliged?.map((row, index) => (
            ...
            <NoDataComponent isVisible={!companiesBuyerPriviliged?.length} />
          </TableBody>
        </Table>
      </StyledTableContainer>
