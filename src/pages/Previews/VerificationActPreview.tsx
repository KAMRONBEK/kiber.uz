...
const VerificationActPreview = () => {
  ...
  const rejectHandler = () => {
    // @ts-ignore
    dispatch(verificationActRejectDoc(docData, params.id, "Sababi nomalum"));
  };
  ...
  const copyHanlder = () => {
    // @ts-ignore
    dispatch(saveVerificationActDoc(docData, docData.verificationActContracts));
  };
      ...
      <div style={{ padding: "20px" }}>
        ...
        <div className="containerChecked">
          ...
          <table cellSpacing="0" style={{ width: "100%" }}>
            ...
            <tbody>
              {/* @ts-ignore */}
              {docData.verificationActContracts.map((item, index) => (
                <>
                  <tr>
                    <td colSpan={8} className="center">
                      №{item.contractNo} от {item.contractDate}
                    </td>
                  </tr>
                  <tr>
                    <td colSpan={2}>Сальдо начальное:</td>
                    <td>{item.openBalance.ownerDebit}</td>
                    <td>{item.openBalance.ownerCredit}</td>
                    <td colSpan={2}>Сальдо начальное:</td>
                    <td>{item.openBalance.partnerDebit}</td>
                    <td>{item.openBalance.partnerCredit}</td>
                  </tr>
                  {/* @ts-ignore */}
                  {item.verificationActContractItems.map((i, index) => (
                    <tr>
                      <td>{i.ownerOperationDate}</td>
                      <td>{i.ownerOperationName}</td>
                      <td>{i.ownerDebit}</td>
                      <td>{i.ownerCredit}</td>
                      <td>{i.partnerOperationDate}</td>
                      <td>{i.partnerOperationName}</td>
                  ...
                  <tr>
                    ...
                    <td>{item.totalBalance.ownerDebit}</td>
                    <td>{item.totalBalance.ownerCredit}</td>
                    <td colSpan={2}>Итого по договору:</td>
                    <td>{item.totalBalance.partnerDebit}</td>
                    <td>{item.totalBalance.partnerCredit}</td>
                  </tr>
                  <tr>
                    <td colSpan={2}>Сальдо конечное по договору:</td>
                    <td>{item.closeBalance.ownerDebit}</td>
                    <td>{item.closeBalance.ownerCredit}</td>
                    <td colSpan={2}>Сальдо конечное по договору:</td>
                    <td>{item.closeBalance.partnerDebit}</td>
                    <td>{item.closeBalance.partnerCredit}</td>
                  </tr>
                </>
              ))}
              <tr>
