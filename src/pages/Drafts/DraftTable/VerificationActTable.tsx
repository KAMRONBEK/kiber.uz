...
const VerificationActTable = () => {
  ...
  const fetchDocList = () => {
    ...
    draftService
      .getDraftsList("verification-act", userTin)
      .then((res) => {
        // @ts-ignore
  ...
  const deleteDraft = (e, id) => {
    ...
    draftService
      .deleteDraft(id, "verification-act")
      .then((res) => {
        // @ts-ignore
          ...
          <TableBody style={{ position: "relative" }}>
            {tableData?.map((row, index) => (
              <StyledTableRow
                ...
                onClick={() =>
                  // @ts-ignore
                  navigate(`/main/verification-act/${row.verificationActId}`)
                }
              >
                <TableCell align="center">{index + 1}</TableCell>
                <TableCell align="center">
                  {/* @ts-ignore */}
                  {t(row.verificationActDoc.verificationActNo)}
                </TableCell>
                <TableCell align="center">
                  <TableTag>
                    {/* @ts-ignore */}
...
export default VerificationActTable;
